...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        if not config['end2end'] and config['is_multimodal_model']:
            feature_dataset = config['feature_dataset'] or config['dataset']
            feature_data_path = config['feature_data_path'] or config['data_path']
            dataset_path = os.path.abspath(os.path.join(feature_data_path, feature_dataset))
            # if file exist?
            ...
            a_feat_file_path = os.path.join(dataset_path, config['audio_feature_file']) \
                if config['audio_feature_file'] else None
            ...
            if os.path.isfile(t_feat_file_path):
                self.t_feat = torch.from_numpy(np.load(t_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)
            if a_feat_file_path and os.path.isfile(a_feat_file_path):
