...
class SMORE(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        if self.v_feat is not None:
            ...
            if os.path.exists(image_adj_file):
                ...
                image_adj = build_sim(self.image_embedding.weight.detach())   #计算余弦相似度
                # 相似度矩阵 → top-k 邻居筛选 → KNN 图 → 图归一化 → 用于后续 GNN 图传播（保留相似度值和自循环）
        ...
        # 归一化后做max:哪个模态传播贡献更大考虑了模态内部度分布	融合后不再是标准归一化图;相比归一化前，可能更合适
        ...
        # 均值 0，方差 1  [batch广播维度, 频率分量数量, 复数的实部和虚部] [1, 33, 2]
    ...
    def pre_epoch_processing(self):
        pass
    #只保留最大值，优：保留更多潜在相关边，避免某个模态漏掉有用邻居；缺：可能引入噪声边，尤其是某个模态的高相似度其实不可靠时。
    ...
    # 把原始特征从普通空间变到频率空间，用可学习的复数权重调节不同频率成分，再变回普通特征空间
    def spectrum_convolution(self, image_embeds, text_embeds):
        """
        Modality Denoising & Cross-Modality Fusion

        image_embeds ──FFT──> image_fft ──乘 image_weight──> IFFT──> image_conv
        text_embeds  ──FFT──> text_fft  ──乘 text_weight ──> IFFT──> text_conv
        image_fft × text_fft × fusion_weight ──IFFT──> fusion_conv
        """
        image_fft = torch.fft.rfft(image_embeds, dim=1, norm='ortho')             #image_fft = [低频成分, 中频成分, 高频成分, ...]  [7050,33]
        ...
        #   Uni-modal Denoising 图像频谱 × 图像频域权重
    ...
    def forward(self, adj, train=False):
        ...
        image_item_embeds = torch.multiply(self.item_id_embedding.weight, self.gate_v(image_conv)) #公式12
        text_item_embeds = torch.multiply(self.item_id_embedding.weight, self.gate_t(text_conv)) #公式12
        fusion_item_embeds = torch.multiply(self.item_id_embedding.weight, self.gate_f(fusion_conv)) #公式13
        ...
        text_user_embeds = torch.sparse.mm(self.R, text_item_embeds)  #用户的直接通过交互矩阵得到
