...
class Coach:
    ...
    def prepareModel(self):
        ...
        out_dims = eval(args.dims) + [args.item]  # [1000, num_item]
    ...
    def trainEpoch(self):
        trnLoader = self.handler.trnLoader  # 加载训练数据
        trnLoader.dataset.negSampling()  # 进行负采样
        epLoss, epRecLoss, epClLoss = 0, 0, 0
        epDiLoss = 0
        epDiLoss_image, epDiLoss_text = 0, 0
        if args.data == 'tiktok':
            epDiLoss_audio = 0
        steps = trnLoader.dataset.__len__() // args.batch  # 一共多少数据除以批次
        ...
        for i, batch in enumerate(diffusionLoader):
            ...
            iEmbeds = self.model.getItemEmbeds().detach()  # torch.Size([7050, 64])
        ...
        for i, tem in enumerate(trnLoader):
            ...
            if args.data == 'tiktok':
                usrEmbeds, itmEmbeds = self.model.forward_MM(self.handler.torchBiAdj, self.image_UI_matrix,
            ...
            # 为对比学习生成需要对比的多模态特征    图像特征Embeds1  文本特征Embeds2
            if args.data == 'tiktok':
            ...
            # 总的用户和图像模态用户
            clLoss1 = (contrastLoss(usrEmbeds, usrEmbeds1, ancs, args.temp) + contrastLoss(itmEmbeds, itmEmbeds1, poss,
                                                                                           args.temp)) * args.ssl_reg
            # 总的用户和文本模态用户
            clLoss2 = (contrastLoss(usrEmbeds, usrEmbeds2, ancs, args.temp) + contrastLoss(itmEmbeds, itmEmbeds2, poss,
                                                                                           args.temp)) * args.ssl_reg
            if args.data == 'tiktok':
                clLoss3 = (contrastLoss(usrEmbeds, usrEmbeds3, ancs, args.temp) + contrastLoss(itmEmbeds, itmEmbeds3,
                                                                                               poss,
                                                                                               args.temp)) * args.ssl_reg
    ...
    def testEpoch(self):
        ...
        for usr, trnMask in tstLoader:
            ...
            log('Steps %d/%d: recall = %.2f, ndcg = %.2f , precision = %.2f   ' % (i, steps, recall, ndcg, precision),
                save=False, oneline=True)
        ret = dict()
        ret['Recall'] = round(epRecall / num, 4)
        ret['NDCG'] = round(epNdcg / num, 4)
        ret['Precision'] = round(epPrecision / num, 4)
        return ret
...
if __name__ == '__main__':
    ...
    log('Start')
    handler = DataHandler()  # 实例化数据处理
    handler.LoadData()  # 加载所有数据
    log('Load Data')
