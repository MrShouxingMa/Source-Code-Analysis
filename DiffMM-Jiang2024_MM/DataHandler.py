...
class DataHandler:
	...
	def makeTorchAdj(self, mat):
		...
		mat = (mat != 0) * 1.0
		mat = (mat + sp.eye(mat.shape[0])) * 1.0  #构造二部图邻接矩阵
		mat = self.normalizeAdj(mat)   			#标准化邻接矩阵

		# make cuda tensor
		idxs = torch.from_numpy(np.vstack([mat.row, mat.col]).astype(np.int64))
		vals = torch.from_numpy(mat.data.astype(np.float32))
		shape = torch.Size(mat.shape)
		return torch.sparse.FloatTensor(idxs, vals, shape).cuda()  #转成tensor格式
	...
	def LoadData(self):
		...
		self.torchBiAdj = self.makeTorchAdj(trnMat)  #制作torch格式的邻接矩阵 [user+item, user+item]
		...
		self.diffusionData = DiffusionData(torch.FloatTensor(self.trnMat.A)) #输入就是训练稀疏矩阵，[user_num. item_num]
...
class TrnData(data.Dataset):
	def __init__(self, coomat):
		...
		self.dokmat = coomat.todok()  #转换成 DOK 格式
