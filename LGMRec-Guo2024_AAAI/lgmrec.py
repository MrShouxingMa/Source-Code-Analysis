...
class LGMRec(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.n_ui_layers = config['n_ui_layers']  # 4
        self.n_hyper_layer = config['n_hyper_layer']  # 1
        self.hyper_num = config['hyper_num']  # 4
        self.keep_rate = config['keep_rate']  # 0.5
        self.alpha = config['alpha']  # 0.3
        self.cl_weight = config['cl_weight']  # 0.0001
        self.reg_weight = config['reg_weight']  # 1e-06
        self.tau = 0.2

        self.n_nodes = self.n_users + self.n_items  # 26495
        # 初始化超图类
        ...
        self.num_inters, self.norm_adj = self.get_norm_adj_mat()  # 得到邻接矩阵 公式2
        ...
        if self.v_feat is not None:
            self.image_embedding = nn.Embedding.from_pretrained(self.v_feat, freeze=True)  # [7050,64]
            self.item_image_trs = nn.Parameter(
                nn.init.xavier_uniform_(torch.zeros(self.v_feat.shape[1], self.feat_embed_dim)))  # [4096,64]
            self.v_hyper = nn.Parameter(
                nn.init.xavier_uniform_(torch.zeros(self.v_feat.shape[1], self.hyper_num)))  # [4096,4]
        if self.t_feat is not None:
            self.text_embedding = nn.Embedding.from_pretrained(self.t_feat, freeze=True)  # [7050,384]
            self.item_text_trs = nn.Parameter(
                nn.init.xavier_uniform_(torch.zeros(self.t_feat.shape[1], self.feat_embed_dim)))  # [384,64]
            self.t_hyper = nn.Parameter(
                nn.init.xavier_uniform_(torch.zeros(self.t_feat.shape[1], self.hyper_num)))  # [384,64]
    ...
    def cge(self):
        ...
        if self.cf_model == 'lightgcn':
            ...
            cge_embs = cge_embs.mean(dim=1, keepdim=False)  # 公式3
        return cge_embs

    # modality graph embedding
    def mge(self, str='v'):
        if str == 'v':
            item_feats = torch.mm(self.image_embedding.weight, self.item_image_trs)  # 公式4
        elif str == 't':
            item_feats = torch.mm(self.text_embedding.weight, self.item_text_trs)  # 公式4
    ...
    def forward(self):
        ...
        if self.v_feat is not None:
            ...
            # 每个 item 到各个 visual hyperedge 的原始打分，转换成一个可微、带随机扰动的“软分配概率”矩阵
            # tau 是温度系数。温度越小，分布越尖锐，越接近 one-hot；温度越大，分布越平滑。
            # hard=False 表示输出不是严格 one-hot，而是连续的 soft probability。
        ...
        if self.v_feat is not None and self.t_feat is not None:
            ...
            all_embs = lge_embs + self.alpha * F.normalize(ghe_embs)  #公式13
        else:
            all_embs = cge_embs
...
class HGNNLayer(nn.Module):
    ...
    def forward(self, i_hyper, u_hyper, embeds):
        ...
        for _ in range(self.h_layer):
            lat = torch.mm(i_hyper.T, i_ret)   #[4, 7050]*[7050, 64]
            i_ret = torch.mm(i_hyper, lat)      #[7050, 4]*[4, 64]
            u_ret = torch.mm(u_hyper, lat)      #[19445, 4]*[4, 64]
        return u_ret, i_ret
